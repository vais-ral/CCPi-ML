...
for layer2 in layer2Neurons:
    for layer1 in layer1Neurons:
        ...
        netData = DataSlice.DataSlice(Features=features,Labels=labels,Shuffle=False,Split_Ratio=1.0)

        netModel = NetSlice.NetSlice(model,'keras',netData)

        routineSettings = {"CompileAll":False,
                            "SaveAll":None}
        routine = [
            {"Compile":[keras.optimizers.Adam(lr=1000),'mean_squared_error',['binary_accuracy', 'categorical_accuracy']],
            "Train":[1,features.shape[0],1]},
            {"Compile":[keras.optimizers.Adam(lr=0.01),'mean_squared_error',['binary_accuracy', 'categorical_accuracy']],
            "Train":[3,features.shape[0],1]},
            {"Compile":[keras.optimizers.Adam(lr=0.001),'mean_squared_error',['binary_accuracy', 'categorical_accuracy']],
            "Train":[4,features.shape[0],1]},
        ]

        netModel.trainRoutine(routineSettings,routine)

        print(model.summary())

        netModel.plotLearningCurve()
        z = netModel.predictModel(features)
        surface.append(z)
        
        netDetailsItem.append(layer1)
        netDetailsItem.append(layer2)
        params.append(netModel.model.count_params())
