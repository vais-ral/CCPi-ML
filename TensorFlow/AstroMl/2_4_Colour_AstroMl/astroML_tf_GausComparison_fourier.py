...
def generateData():
    ...
    X_train, y_train = reBalanceData(X_train,y_train,0.3)
    ...
    X_test, y_test = reBalanceData(X_train,y_train,0.3) 
...
#		X=np.append(X,np.multiply(X[:,[i]],X[:,[i]]),axis=1)
#
#	print(X_train[0])
#    
...
#		X_train = X_train[:, [1,0,5,4]]  # rearrange columns for better 2-color results
#		X_test = X_test[:, [1,0,5,4]]  # rearrange columns for better 4-color results
#		X = X[:, [1,0,5,4]]
#		X_test_unbalanced = X_test_unbalanced[:, [1,0,5,4]]  # rearrange columns for better 2-color results
		...
		X_train = X_train[:, [1,0,2,5,4,6]]  # rearrange columns for better 3-color results
		X_test = X_test[:, [1,0,2,5,4,6]]  # rearrange columns for better 4-color results
		X = X[:, [1,0,2,5,4,6]]
		X_test_unbalanced = X_test_unbalanced[:, [1,0,2,5,4,6]]  # rearrange columns for better 2-color results

	elif coll==3:
		X_train = X_train[:, [1,0,2,3,5,4,6,7]]  # rearrange columns for better 4-color results
		X_test = X_test[:, [1,0,2,3,5,4,6,7]]  # rearrange columns for better 4-color results
		X = X[:, [1,0,2,3,5,4,6,7]]
		X_test_unbalanced = X_test_unbalanced[:, [1,0,2,3,5,4,6,7]]  # rearrange columns for better 2-color results

    
	X_train = fft(X_train)
	X_test = fft(X_test)
	X = fft(X)
	X_test_unbalanced = fft(X_test_unbalanced)
	#X_train,X_test,y_train,y_test = splitdata(X,y,0.7)
	...
	if coll == 1 :
		...
		yshape = int((ylim[1]-ylim[0])*1000)+1
